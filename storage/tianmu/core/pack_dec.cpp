...
size_t PackDec::CalculateMaxLen() const {
  return *std::max_element(data.lens8, data.lens8 + dpn->numOfRecords);
}
...
std::pair<PackDec::UniquePtr, size_t> PackDec::Compress() {
  ...
  if (dpn->numOfNulls > 0) {
    comp_null_buf_size = ((dpn->numOfRecords + 7) / 8);
    ...
    CprsErr res = bsc.Compress(comp_null_buf.get(), comp_null_buf_size, (char *)nulls.get(), dpn->numOfRecords, dpn->numOfNulls);
...
void PackDec::Save() {
  ...
  if (!ShouldNotCompress()) {
    if (data.sum_len > common::MAX_CMPR_SIZE) {
      ...
      dpn->dataLength = bitmapSize + data.sum_len + (data.len_mode * (1 << s->pss));
    } else if (s->ColType().GetFmt() == common::PackFmt::TRIE) {
      CompressTrie();
    } else {
      auto res = Compress();
...
void PackDec::LoadCompressed(system::Stream *f) {
  ...
  if (dpn->numOfNulls > 0) {
    ...
    for (uint i = 0; i < dpn->numOfRecords; i++) tmp_index[i] = FREE_PLACE;
...
types::BString PackDec::GetValueBinary(int ono) const {
  ...
  DEBUG_ASSERT(ono < (int)dpn->numOfRecords);
